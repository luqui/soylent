...
public:
	...
	void step(GPUFluid* field) {
		...
		float push = 2 * (radius > 5 ? 5 : radius);
		for (int i = 0; i < 40; i++) {
			...
			if (splode.x > CLAMPW && splode.x < W-CLAMPW-1 && splode.y > CLAMPH && splode.y < H-CLAMPH-1) {
				field->add_velocity(splode, -push * ~(vec(-p.y,p.x)+p), 3.0/W);
				field->add_density(splode, power_, 3.0/W);
			}
		}
	}
...
private:
	...
	void put_circle(GPUFluid* field, float radius) {
		/*for (int i = 0; i < 80; i++) {
		...
		field->add_density(pos_, power_, radius_);
	}

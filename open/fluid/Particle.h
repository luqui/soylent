...
public:
	...
	void step(GPUFluid* field) {
		...
		vec grad;
		grad.x = (fabs(field->get_density(pos + vec(1,0))) - fabs(field->get_density(pos + vec(-1,0)))) / 2.0;
		grad.y = (fabs(field->get_density(pos + vec(0,1))) - fabs(field->get_density(pos + vec(0,-1)))) / 2.0;
		pos -= 0.001 * PARTICLEDRIFT * DT * ~grad;

		grad.x = (fabs(field->get_density(pos + vec(1,0))) - fabs(field->get_density(pos + vec(-1,0)))) / 2.0;
		grad.y = (fabs(field->get_density(pos + vec(0,1))) - fabs(field->get_density(pos + vec(0,-1)))) / 2.0;
		pos -= 0.001 * PARTICLEDRIFT * DT * ~grad;//do it twice so that they move more but aren't jittery;
	}

...
private:
	...
	void frame_input() {
		...
		ppos_.set_brake(posinput_->get_brake());
		pneg_.set_brake(neginput_->get_brake());
	...
	void check_particles() {
		...
		while (particles_pending_ > 1) {
			...
			//field_->set_density(pos, field_->get_density(pos) / 1.2);  **Took this out because we don't want to subtract density for each dot*?*
...
public:
	DuelMode(Input* posinput, Input* neginput) 
		: ppos_(Color(1, 0.5, 0), 1, vec(float(W)/10.0, float(H - float(H)/10.0)), 5, load_texture(ICHOR_DATADIR "/redfirefly.png")),
		  pneg_(Color(0, 0.5, 1), -1,vec(float(W-float(W)/10.0), float(H)/10.0), 5, load_texture(ICHOR_DATADIR "/bluefirefly.png")),
		  invincible_timer_(INVINCIBLE_TIME),

 ...
 * @file streamopGL.h
 * 
 * StreamOp Policies for OpenGL.
 */
#ifndef __STREAMOPGL_H__
#define __STREAMOPGL_H__
 ...
 * @brief Handles StreamOp rendering (a quad) with a single texture coordinate.
 */
class SimpleGLComputePolicy
{
public:
  ...
  //--------------------------------------------------------------------------
  // Function     	    : SetStreamRect
  // Description	    : 
  //--------------------------------------------------------------------------
  /**
   * @fn SetStreamRect(float minX, float minY, float maxX, float maxY, float z)
   * @brief Sets the dimensions of the quad to render.
   */ 
  void SetStreamRect(float minX, float minY, float maxX, float maxY, float z = 0.5f)
  {
...
protected:  // methods
  ...
  SimpleGLComputePolicy() : _rMinX(0), _rMinY(0), _rMaxX(1), _rMaxY(1), _rZ(0.5f)
  {
  }
  ...
  /**
   * @fn Compute()
   * @brief Performs the Stream computation.  Called by StreamOp::Compute()
   */ 
  void Compute()
  {
    glBegin(GL_QUADS);
...
public:
  ...
  //--------------------------------------------------------------------------
  /**
   * @fn SetStreamRect(float minX, float minY, float maxX, float maxY)
   * @brief Sets the dimensions of the quad to render.
   */ 
  void SetStreamRect(float minX, float minY, float maxX, float maxY, float z = 0.5f)
  {
...
protected:  // methods
  ...
  SingleTextureGLComputePolicy() : _rMinX(0), _rMinY(0), _rMaxX(1), _rMaxY(1), _rZ(0.5f) 
  {
  ...
  void Compute()
  {
    glBegin(GL_QUADS);
    {
      ...
      glTexCoord2f(_rMaxS, _rMinT); glVertex3f(_rMaxX, _rMinY , _rZ);
      glTexCoord2f(_rMaxS, _rMaxT); glVertex3f(_rMaxX,  _rMaxY, _rZ);
 ...
 * @brief Handles StreamOp rendering (a quad) with multiple texture coordinates.
 */
class MultiTextureGLComputePolicy
{
public:
...
protected:  // methods
  ...
  MultiTextureGLComputePolicy() : 
            _rMinX(0), _rMinY(0), _rMaxX(1), _rMaxY(1), _rZ(0.5)
  {
    glGetIntegerv(GL_MAX_TEXTURE_UNITS, &_iNumTextureUnits);
  ...
  void Compute()
  {
    glBegin(GL_QUADS);
    {
      ...
      for (i = 0; i < _iNumTextureUnits; ++i)
        glMultiTexCoord2f(GL_TEXTURE0 + i, _rMinS[i], _rMinT[i]);
      glVertex3f(_rMinX, _rMinY, _rZ);
      // LR
      for (i = 0; i < _iNumTextureUnits; ++i)
        glMultiTexCoord2f(GL_TEXTURE0 + i, _rMaxS[i], _rMinT[i]);
      glVertex3f(_rMaxX, _rMinY, _rZ);
      // UR
      for (i = 0; i < _iNumTextureUnits; ++i)
        glMultiTexCoord2f(GL_TEXTURE0 + i, _rMaxS[i], _rMaxT[i]);
      glVertex3f(_rMaxX,  _rMaxY, _rZ);
      // UL
      for (i = 0; i < _iNumTextureUnits; ++i)
        glMultiTexCoord2f(GL_TEXTURE0 + i, _rMinS[i], _rMaxT[i]);
      glVertex3f(_rMinX,  _rMaxY, _rZ);
    }
    glEnd();
  }
 ...
 * @brief Renders only the boundaries (one texel width) of a Stream.
 */
class BoundaryGLComputePolicy
{
public:
...
protected:  // methods
  ...
  : _iTexResS(1), _iTexResT(1), _rMinX(0), _rMinY(0), _rMaxX(1), _rMaxY(1), _rZ(0.5f),
  ...
  /**
   * @fn Compute()
   * @brief Performs the Stream computation on the Stream boundaries.  Called by StreamOp::Compute()
   * 
   * This renders the four 1-pixel wide boundaries of the Stream.
   */ 
  void Compute()
  {
    ...
    {      
      ...
      if (_bBottom)
      {
        glMultiTexCoord2f(GL_TEXTURE1, 0, 
                          _bBottomPeriodic ? _iTexResT - 2 * _rTexelHeight : 
                                             _rTexelHeight); // offset amount
        glTexCoord2f(_rMinS, _rMinT); 
        glVertex3f(_rMinX, _rMinY + 0.5f * _rPixelHeight, _rZ);
        glTexCoord2f(_rMaxS, _rMinT); 
        glVertex3f(_rMaxX, _rMinY + 0.5f * _rPixelHeight, _rZ);
      }
      // top boundary
      if (_bTop)
      {
        ...
        glVertex3f(_rMinX, _rMaxY - 0.5f * _rPixelHeight, _rZ); 
        glTexCoord2f(_rMaxS, _rMaxT - _rTexelHeight); 
        glVertex3f(_rMaxX, _rMaxY - 0.5f * _rPixelHeight, _rZ);
      }
    }
    glEnd();
    /*glBegin(GL_QUADS);
 ...
 * @brief Handles the copy to texture needed by some Stream Operations.
 */
class CopyTexGLUpdatePolicy
{
public:
  ...
  void SetEnableCopy(bool bEnable) { _bEnableCopy = bEnable; }
  bool GetEnableCopy() const       { return _bEnableCopy;    }

protected: // methods
  CopyTexGLUpdatePolicy() 
    : _iOutputTexture(0), _iXOffset(0), _iYOffset(0),
      _iX(0), _iY(0), _iWidth(0), _iHeight(0), _bEnableCopy(true) {}
  ...
  //--------------------------------------------------------------------------
  /**
   * @fn UpdateOutputStream()
   * @brief Updates the output texture.  Called by StreamOp::Compute()
   */ 
 ...
 * @brief Handles the copy to a 3D texture slice needed by some Stream Operations.
 */
class Copy3DTexGLUpdatePolicy
{
public:
...
protected: // methods
  ...
  /**
   * @fn UpdateOutputStream()
   * @brief Updates a slice in the output texture. Called by StreamOp::Compute()
   */ 
  void UpdateOutputStream()
  {

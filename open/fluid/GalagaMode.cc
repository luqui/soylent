...
private:
	...
	void frame_input() {
		...
		player_->set_brake(input_->get_brake());
	}
	...
	void init() {
		net_lives_ = 1;
		player_ = new Dueler(Color(1, 0.5, 0), 1, vec(W/2, H/2), net_lives_, load_texture(ICHOR_DATADIR "/redfirefly.png"));
		...
		explosions_.push_back(new DensityGlobExplosion(vec(W/2,H/2), 0.2, 2.5, .15, 1));
		init_color();
	}
	...
	void spawn_animation(vec pos) {
		...
		if (power > 0) power = 0;
		explosions_.push_back(new DensityGlobExplosion(pos, 0.2, power, .15, 1));
	}

	template <class E>
	E* spawn_enemy(float diff = 1) {
		vec pos = generate_spawn_position();
		spawn_animation(pos);
		E* e = new E(pos, 0.3, diff);
	...
	void step() {
		...
		for (std::list<Enemy*>::iterator i = enemies_.begin(); i != enemies_.end();) {
			if ((*i)->dead(this)) {
				...
				if (int((score_+100)/(100*net_lives_)) > net_lives_) {
					net_lives_++;
					player_->extra_life();
				}
				...
				if ((*i)->should_explode()) {
					if (combo_ < 10) {
						explosions_.push_back(new DensityGlobExplosion((*i)->position(), sqrt(float(combo_))*0.2, 2, sqrt(float(combo_))*0.05));
					}
					else {
						explosions_.push_back(new DensityGlobExplosion((*i)->position(), sqrt(float(10))*0.2, 2, sqrt(float(10))*0.05));
					}
				}
				prune_joints(*i);
				delete *i;
			...
			for (std::list<Enemy*>::iterator j = enemies_.begin(); j != enemies_.end(); ++j) {
				if (i == j) continue;
				if (((*i)->position() - (*j)->position()).norm() < 20) {
					if (!joint_exists(*i, *j)) {
						Joint joint(*i, *j);
...
private:
	...
	class PatternSpawner: public Level {
	public:
		PatternSpawner(GalagaMode* mode): mode_(mode) { }
		void step() {
			int total_score = 0;
			for (std::list<Enemy*>::const_iterator i = mode_->enemies_.begin(); i != mode_->enemies_.end(); ++i) {
				total_score += (*i)->par_score();
			}

			int score_left = 1 + sqrt(float(mode_->score_))*2 - total_score;
			while (score_left > 0) {
				Enemy* e = 0;
				switch (rand() % 6) {
				case 0: e = mode_->spawn_enemy<BasicEnemy>(); break;
				case 1: e = mode_->spawn_enemy<SpinEnemy>(); break;
				case 2: e = mode_->spawn_enemy<MissileEnemy>(); break;
				case 3: e = mode_->spawn_PairBomb_pair(mode_->generate_spawn_position()); break;
				case 4: e = mode_->spawn_enemy<ScorpionEnemy>(); break;
				case 5: e = mode_->spawn_enemy<VortexEnemy>(); break;
				}
				if (e) score_left -= e->par_score();
			}
		}

		bool advance() const { return false; }
	private:
		GalagaMode* mode_;
	};
...
#define MAKE_RANDOM_SERIES(NAME, ENEMIES, RATE, CODE) \
	...
	MAKE_RANDOM_SERIES(Level1, 100, 2, {
	...
	Level* make_level(int num) {
		return new PatternSpawner(this);
	}
	...
	public:
		...
		void step(GalagaMode* mode) {
			...
			pos_ += mode->field_->get_velocity(pos_) * 7 * DT;
			vec dir = ~(mode->player_->position() - pos_);
			mode->field_->add_density(pos_, -difficulty_ * 5);
			mode->field_->add_velocity(pos_ - 5*dir, 5*dir);
		}
	...
	public:
		...
		void step(GalagaMode* mode) {
			...
			mode->field_->add_density(pos_, -8);
			add_wide_velocity(mode->field_, pos_, 5*dir);
			pos_ += (mode->field_->get_velocity(pos_) + 5*dir) * 20 * DT;
		}
	...
	public:
		...
		void step(GalagaMode* mode) {
			...
			vec dir = ~(mode->player_->position() - pos_);
			mode->field_->add_density(pos_ + 5*dir, -10);
			mode->field_->add_velocity(pos_, 5*dir);
			pos_ -= 100 * grad * DT;
		}
	...
	public:
		...
		void step(GalagaMode* mode) {
			...
			add_wide_velocity(mode->field_, pos_, 6*dir);
			pos_ += mode->field_->get_velocity(pos_) * 5 * DT;
		}
		...
		float lifetime() const { return lifetime_; }
		int par_score() const { return should_explode() ? 10 : 0; }
	...
	PairBomb* spawn_PairBomb_pair(vec apos, float difficulty = 1) {
		vec bpos;
		int safety = 0;
		do {
			if (safety++ > 50) break;
		...
		if ((apos - bpos).norm() < 150) return 0;
		...
		void step(GalagaMode* mode) {
			...
			for (int i = 0; i < 96; i+= 3) {
				...
				if (direction > 0) {  // expanding
					...
					mode->field_->add_density(src, -5);
				}
				mode->field_->add_velocity(src, 5*direction*~dir);
			}
			pos_ += mode->field_->get_velocity(pos_) * 7 * DT;
			pos_.x = clamp(pos_.x, CLAMPW, W - CLAMPW - 1);
			pos_.y = clamp(pos_.y, CLAMPH, H - CLAMPH - 1);
	...
	public:
		...
		void step(GalagaMode* mode) {
			...
			mode->field_->add_density(pos_, -10);
			if (largest > -1e-4) {
				angle_ = atan2(largestPos.y, largestPos.x);
			}
			
			add_wide_velocity(mode->field_, pos_, 6*vec(cos(angle_),sin(angle_)));
			pos_ += mode->field_->get_velocity(pos_)* 6 * DT;
